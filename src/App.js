...
class App extends React.Component {
  ...
  getMovies(e) { // get the movie list
    if (e.target.value.length >= 3) {
      fetch(`https://www.omdbapi.com/?i=tt3896198&apikey=498cb491&page=1&s=${e.target.value}`)
  ...
  render() {
    return (
      <div id="containerRef">
        ...
        <input type="text" onClick={(event) => this.getMovies(event)} onChange={(event) => this.getMovies(event)} />
        {this.selectedMovies()}
        <ul id="movieList">
          {this.renderLi()}
        </ul>
